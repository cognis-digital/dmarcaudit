...
- **rules/analyzers** apply the heuristics shipped in `dmarcaudit/core.py`.
- **score** ranks by severity.
- **MCP server** (`dmarcaudit mcp`) exposes `scan` for Cognis.Studio agents.
